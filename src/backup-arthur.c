...
/** Semaforos */
SemaphoreHandle_t xSemaphoreCrash;
...
/* Inicializa Função de fusão */
  ...
  for (;;) {
    ...
    // Aceleração
    float acc = sqrt(pow( proc_acc_x, 2) + pow( proc_acc_y, 2) + pow( proc_acc_z, 2));
    if (acc < 0.5){
        printf("batida\n");
        xSemaphoreGive(xSemaphoreCrash);
    }
    ...
    if (euler.angle.yaw > 60){
      ori = DIREITA;
      xQueueSend(xQueueYaw, &ori, (TickType_t) 0);
    }
...
static void task_house_down(void *pvParameters) {
  for (;;) {
    if (xSemaphoreTake(xSemaphoreCrash, 0)){
      for(int i = 0; i < 2; i++){
...
static void task_orientacao(void *pvParameters) {
  ...
  enum orientacao ori;
  for (;;) {
    if (xQueueReceive(xQueueYaw, &ori, 1)) {
			if (ori == ESQUERDA){
...
int main(void) {
  ...
  xSemaphoreCrash = xSemaphoreCreateBinary();
	if (xSemaphoreCrash == NULL)
		printf("falha em criar o semaforo de batida \n");

  xQueueYaw = xQueueCreate(10, sizeof(float));
  	if (xQueueYaw == NULL)
    	printf("falha em criar a queue Yaw \n");
  ...
  if (xTaskCreate(task_house_down, "house_down", TASK_LED_STACK_SIZE, NULL,
                  TASK_LED_STACK_PRIORITY, NULL) != pdPASS) {
    printf("Failed to create test imu task\r\n");
  }
